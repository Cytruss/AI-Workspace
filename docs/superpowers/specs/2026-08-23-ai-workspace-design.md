...
Each adapter owns executable discovery, arguments, structured-output parsing, model-execution observation, and provider-specific permission settings. Setup records detected capabilities, but every run validates critical read-only capabilities again so an upgraded or replaced executable cannot silently weaken policy. Model, effort, and observed-model-reporting capabilities are independent: safely knowable effort values may be reported, but no adapter implies that requested effort was observed. When `requestedEffort` is present, `effortOption.allowedValues` must exist and contain it; otherwise the request fails before process creation with `AGENT_EFFORT_UNSUPPORTED`. Omitted effort does not require an allowed-values list.
...
Claude requires `--bare`, `--settings <compact-inline-json>`, `--tools "Read,Glob,Grep"`, `--disallowedTools "mcp__*"`, `--permission-mode plan`, `--no-session-persistence`, `-p`, `--output-format json`, and `--json-schema <compact-inline-json>`. The settings value is exactly the bounded JSON object `{"fallbackModel":[],"switchModelsOnFlag":false}` and is passed as one argument without a shell. Together with `--bare`, it neutralizes ordinary user, shared-project, and project-local fallback configuration. Anthropic managed or server-managed settings have higher precedence and can override inline settings, so AI Workspace does not promise pre-execution fallback prevention or avoidance of provider work/cost. `--bare` disables discovered MCP servers and customizations, while explicit MCP denial is defense in depth. Bash, Edit, Write, Notebook, and every built-in tool other than Read, Glob, and Grep are unavailable. The compact response-schema JSON is bounded and passed inline as one argument; it is never written to the Codex schema file or interpreted by a shell.
...
After Claude execution, the adapter parses actual models from JSON `modelUsage`, normalizes them to a sorted unique array, and requires observations for an explicit selection. Every observed ID must match an exact ID or literal prefix in the configured selected-class policy; no operator string is interpreted as a regular expression. A mismatch fails as `MODEL_CLASS_CHANGED`; missing observations fail as `MODEL_OBSERVATION_UNAVAILABLE`. Both retain bounded diagnostics and attempted `ModelExecution` data and are never rendered as valid results, but any managed-policy fallback work and cost have already occurred. Provider-default calls persist all observations and remain `unverified`. Aliases may resolve to newer versions inside the same configured class; full CLI IDs plus exact accepted IDs provide operator-controlled pinning. Requested effort is persisted but is not described as observed.
...
Agent executable resolution is portable and deterministic. A configured explicit path is validated first, then the process `PATH` is searched without invoking a shell, then only narrowly documented native candidates are checked. Windows accepts direct `.exe` files only, including `%APPDATA%\npm\node_modules\@anthropic-ai\claude-code\bin\claude.exe` for npm installs and the provider-documented `%USERPROFILE%\.local\bin\claude.exe`; `.cmd` and `.bat` paths are never selected, persisted, or executed. A discovered `%APPDATA%\npm\claude.cmd` shim is diagnostic evidence only and yields an actionable native-target suggestion. macOS and Linux candidates remain native executable files such as the provider-documented `~/.local/bin/claude`. Every candidate must be a regular executable file and pass a direct bounded-runner `--version` spawn with an argument array before use. The resolver never recursively scans a home directory, parses a shim, reads credentials or provider sessions, invokes a shell, or mutates `PATH`. Setup may offer to persist a verified native path only after explicit confirmation; doctor reports the attempted resolution order and an actionable command/path diagnostic.
...
9. The adapter normalizes the response and `ModelExecution`. An explicit Claude selection is successful only when every post-execution `modelUsage` ID verifies as the selected class. Persistence records successes and model-verification failures before Discord formatting begins; rejection cannot reverse provider work or cost.
...
- Ordinary Claude user/project/local fallback settings are neutralized, but managed policy can override them and execute fallback. Post-execution cross-class observation fails as `MODEL_CLASS_CHANGED`; a missing explicit observation fails as `MODEL_OBSERVATION_UNAVAILABLE`. Either may follow billable provider work.
- An explicit requested effort fails as `AGENT_EFFORT_UNSUPPORTED` before spawn unless the probed bounded allowed-values list exists and contains it.
...
Unit tests cover configuration selection/observation-policy count, duplicates, defaults, literal-prefix matching, separate provider model selection, normalized request/result model contracts, effort allowed-value absence/membership, argument arrays, authorization, native executable candidates on Windows/macOS/Linux, `.cmd`/`.bat` rejection, path validation, all three phase-discriminated schemas, exact board coverage, other-provider claims, every local/canonical ID namespace error, canonical claim/evidence assignment, cross-provider ID reuse, within-run collisions, duplicate merges and origin preservation, new-evidence translation, deliberation state transitions, the exhaustive verdict matrix, mechanical evidence resolution, immutable polishing checks, bounded context, error mapping, formatting, and redaction. Integration tests cover unknown selection and unsupported effort rejection before spawn, unchanged selection propagation through every debate call, persisted `ModelExecution`, accepted same-class alias version changes, post-execution managed-like cross-class and absent-observation failures, Claude `modelUsage` normalization, ordinary user/project/local fallback-settings neutralization, direct bounded-runner native executable probing, SQLite migrations and complete phase-specific call reconstruction, orphan/cross-board evidence rejection, ambient MCP denial, process-tree cancellation, timeouts, output limits, and both CLI adapters' three structured-output schemas and distinct schema transports. Discord tests use a transport boundary rather than a live server.
