...
- Produces: `AppConfig`, `ProjectConfig`, `AgentConfig`, `ModelSelection`, `DebateConfig`, `getAppPaths()`, `loadConfig()`, and `saveConfig()`.
...
CREATE TABLE evidence_references (
  board_id TEXT NOT NULL, session_id TEXT NOT NULL, canonical_id TEXT NOT NULL,
  ...
  PRIMARY KEY (board_id, canonical_id),
  UNIQUE (board_id, canonical_id, session_id),
  FOREIGN KEY (board_id) REFERENCES claim_boards(id),
  FOREIGN KEY (board_id, session_id) REFERENCES claim_boards(id, session_id)
);
CREATE TABLE evidence_origins (
  id TEXT PRIMARY KEY, board_id TEXT NOT NULL, session_id TEXT NOT NULL,
  canonical_evidence_id TEXT NOT NULL,
  agent_id TEXT NOT NULL, agent_run_id TEXT NOT NULL REFERENCES agent_runs(id),
  provider_local_id TEXT NOT NULL,
  FOREIGN KEY (board_id, canonical_evidence_id, session_id)
    REFERENCES evidence_references(board_id, canonical_id, session_id),
...
CREATE TABLE stances (
  ...
  FOREIGN KEY (board_id, canonical_claim_id) REFERENCES claims(board_id, canonical_id),
  UNIQUE (canonical_claim_id, round_id, agent_id), UNIQUE (id, board_id)
);
CREATE TABLE stance_evidence (
  stance_id TEXT NOT NULL,
  board_id TEXT NOT NULL, canonical_evidence_id TEXT NOT NULL,
  FOREIGN KEY (stance_id, board_id) REFERENCES stances(id, board_id),
  FOREIGN KEY (board_id, canonical_evidence_id) REFERENCES evidence_references(board_id, canonical_id),
...
Serialize claim-board snapshots and verdicts with canonical key ordering, enforce configured board byte/claim limits before insertion, and verify their SHA-256 content hashes on load. Persist every exact phase-discriminated provider response in its producing run before persisting the translated output board and rendered report. Guard invalid transitions and all cross-session/board links in one transaction; database foreign keys must reject orphan evidence boards, origins from runs in another session, and claim/stance evidence joins across boards. Constrain stances to `ACCEPT`, `DISPUTE`, or `UNCERTAIN`, evidence resolution to `VERIFIED`, `INVALID`, or `MISSING`, and verdict classification to `CONSENSUS`, `DISAGREEMENT`, `REJECTED`, or `UNRESOLVED`. Generate opaque record IDs with `randomUUID()`; canonical claim and evidence IDs are assigned only by Task 9.
...
```ts
ProviderClaimLocalId = string & Brand<"ProviderClaimLocalId">;
ProviderEvidenceLocalId = string & Brand<"ProviderEvidenceLocalId">;
CanonicalClaimId = string & Brand<"CanonicalClaimId">;
CanonicalEvidenceId = string & Brand<"CanonicalEvidenceId">;
ProviderEvidenceDraft = { localId: ProviderEvidenceLocalId; trackedPath: string; lineStart?: number; lineEnd?: number; contentHash?: string };
InitialClaimDraft = { localId: ProviderClaimLocalId; text: string; material: boolean; evidenceLocalIds: ProviderEvidenceLocalId[] };
InitialPhaseResponse = { phase: "initial"; claims: InitialClaimDraft[]; evidence: ProviderEvidenceDraft[] };
ProviderStanceDraft = { claimId: CanonicalClaimId; value: "ACCEPT" | "DISPUTE" | "UNCERTAIN"; reasoning: string; existingEvidenceIds: CanonicalEvidenceId[]; newEvidenceLocalIds: ProviderEvidenceLocalId[] };
CrossExaminationPhaseResponse = { phase: "cross-examination"; stances: ProviderStanceDraft[]; newEvidence: ProviderEvidenceDraft[] };
FinalPhaseResponse = { phase: "final"; stances: ProviderStanceDraft[]; newEvidence: ProviderEvidenceDraft[] };
ProviderPhaseResponse = InitialPhaseResponse | CrossExaminationPhaseResponse | FinalPhaseResponse;
ClaimOrigin = { agentId: AgentId; agentRunId: string; providerLocalId: string };
CanonicalClaim = { id: CanonicalClaimId; text: string; material: boolean; evidenceIds: CanonicalEvidenceId[]; origins: ClaimOrigin[] };
EvidenceOrigin = { agentId: AgentId; agentRunId: string; providerLocalId: string };
CanonicalEvidence = { id: CanonicalEvidenceId; status: "VERIFIED" | "INVALID" | "MISSING"; trackedPath: string; lineStart?: number; lineEnd?: number; expectedHash?: string; resolvedHash?: string; origins: EvidenceOrigin[] };
...
Provider default is represented only by an absent `AgentRequest.modelSelection`, never a partially null selection. `resolveModelSelection` returns one immutable object or `undefined`, rejects unknown/disallowed classes before argument building or process spawn, and copies `class`, `cliModelId`, and `requestedEffort` without provider-specific renaming. Before argument building, `validateModelCapabilities` requires model-flag support for every explicit selection and, when `requestedEffort` exists, requires `effortOption.supported`, a bounded `allowedValues` list, and exact membership; otherwise it throws `AGENT_EFFORT_UNSUPPORTED`. Omitted effort does not require known allowed values. `ModelExecution.observedModelIds` is bounded, normalized, sorted, and deduplicated. Requested effort is never represented as observed effort.
...
For each adapter, table-test the exact argument array for provider default, explicit concrete class without effort, and explicit concrete class with effort. Assert metacharacters remain one inert array value. Unknown/disallowed classes, missing effort allowed-values, and effort values outside the probed list are rejected before either argument builder or process runner is called; use `AGENT_EFFORT_UNSUPPORTED` for both effort failures. Table-test all three response schemas through both providers: the transported schema has the requested discriminant and reserved fields, and a valid response from another phase is rejected. Claude tests prove inline settings override ordinary user, shared-project, and project-local `fallbackModel` and `switchModelsOnFlag` values, while a fake managed-like fallback can still execute and is rejected post-execution from `modelUsage`. Same-class version IDs pass literal exact/prefix matching; `opus` observing `sonnet` and `fable` observing `opus` fail `MODEL_CLASS_CHANGED`; absent `modelUsage` for an explicit selection fails `MODEL_OBSERVATION_UNAVAILABLE`. Assert these failures occur after the fake provider call and retain usage/cost diagnostics. Provider-default results preserve observations with `unverified` verification.
...
`probe()` runs the separately installed `<command> --version` and `<command> --help` through `runProcess`, rejects unparseable versions below `CLAUDE_MINIMUM_HARDENED_VERSION = "2.1.233"`, and fails closed unless every mandatory safety, ambient-isolation, session, settings, model, effort, model-observation, and structured-output capability is present. This conservative reviewed compatibility floor covers the complete locally verified flag set and the documented `fallbackModel`, `switchModelsOnFlag`, and JSON `modelUsage` contracts; changing it requires updated fixtures and integration evidence. Probe effort allowed values when a safe non-inference contract exposes them; an explicit effort is unusable when that list is absent. `run()` receives a pre-resolved and capability-validated selection and uses the same integrity snapshots as Codex but creates no schema file: it compacts the selected phase JSON schema, enforces the 32,768-byte UTF-8 bound, and supplies it as exactly one `--json-schema` argument. The argument builder emits `--bare`, `--settings`, the compact single-argument value `{"fallbackModel":[],"switchModelsOnFlag":false}`, `--tools`, `Read,Glob,Grep`, `--disallowedTools`, `mcp__*`, `--permission-mode`, `plan`, `--no-session-persistence`, `-p`, `--output-format`, `json`, `--json-schema`, and the compact schema value, plus direct model/effort elements only for an explicit selection; the prompt travels through bounded stdin and no shell interprets any value. Pass only `ANTHROPIC_API_KEY`, `CLAUDE_CONFIG_DIR`, and runtime-critical environment names.

`--bare` disables discovered MCP servers and customizations; `--disallowedTools "mcp__*"` is required defense in depth against ambient MCP tools. Inline settings replace ordinary user, shared-project, and project-local fallback configuration: the empty `fallbackModel` chain and `switchModelsOnFlag:false` are the requested session values. Managed or server-managed Anthropic settings have higher precedence and may override them, so do not claim that fallback execution or cost is prevented. Only Read, Glob, and Grep are available. Do not pass or expose Bash, Edit, Write, Notebook, `--fallback-model`, `--dangerously-skip-permissions`, `--allow-dangerously-skip-permissions`, `--allowedTools`, or edit-capable permission modes.

After the process completes, parse `modelUsage` keys into a bounded sorted unique `observedModelIds` array. For an explicit selection, require at least one observation and require every ID to equal an accepted exact ID or start with an accepted literal prefix from that class's configuration. On mismatch return failed `MODEL_CLASS_CHANGED`; on absence return failed `MODEL_OBSERVATION_UNAVAILABLE`. Preserve the attempted `ModelExecution`, raw bounded provider diagnostics, observations, and safe usage/cost metadata in both failures, and never format their content as a valid selected-model result. This validation cannot undo provider work or cost. Alias movement to a newer version within the accepted class is valid; exact pinning uses a full `cliModelId` and exact accepted IDs. Provider-default results are always `unverified`. Effort remains requested-only, and absent/non-member allowed values fail the capability gate before spawn.

- [ ] **Step 5: Add fake-process integration coverage**

Inject executable names, separate argument builders, and the process runner. Exercise both complete adapter lifecycles with fake Node CLIs while asserting fail-closed version/capability checks for every required flag, effort allowed-value validation before spawn, prompt stdin, all three structured phase schemas, bounded diagnostics, output-limit termination, actionable unsupported/unauthorized model errors without application fallback, normalized `ModelExecution`, unchanged Git state, and complete descendant-tree cancellation. Codex tests assert the selected restrictive phase schema file path occupies the `--output-schema` value and that the one Codex temporary file is removed in `finally`. Claude tests assert the selected compact phase schema occupies exactly one inline `--json-schema` array element, no schema file is created or removed, the exact fallback-neutralizing settings JSON occupies one `--settings` element despite conflicting ordinary user/project/local settings, the exact Read/Glob/Grep allowlist is present, `--bare` and explicit `mcp__*` denial are both present, and Bash/Edit/Write/Notebook plus ambient MCP tools cannot be invoked. Fake results exercise `modelUsage` normalization, same-class alias-version acceptance, missing observation, post-execution managed-like cross-class fallback with retained usage/cost diagnostics, classifier refusal, and availability failure. Both tests assert the unchanged resolved model/effort values are direct inert arguments and provider default omits them. The same integration suite must be runnable on Windows, macOS, and Linux CI.
...
**Interfaces:**
- Consumes: `AgentRegistry`, Task 2 concrete model selections, `ProjectService`, `ProjectRepository`, and `SessionRepository`.
...
**Interfaces:**
- Consumes: `AgentRegistry`, Task 2 `DebateConfig` and concrete model selections, Task 6 structured schemas, `ProjectService`, `SessionRepository`, `DeliberationRepository`, `ActiveRuns`, and `ConcurrencyGate`.
...
- explicit Claude selections accept newer observed IDs only within the configured concrete class, reject a different class after execution with `MODEL_CLASS_CHANGED`, reject absent `modelUsage` with `MODEL_OBSERVATION_UNAVAILABLE`, and retain safe usage/cost diagnostics because managed-policy fallback may already have incurred provider work; provider-default runs preserve observations as `unverified`;
- explicit effort with absent or non-member probed allowed values fails as `AGENT_EFFORT_UNSUPPORTED` before any debate process starts, while omitted effort remains valid;
...
Require `InitialPhaseResponse` from both initial calls. Normalize its independently produced provider-local claim and evidence drafts through Task 6 schemas. Reject duplicate provider-local claim or evidence IDs within one run, dangling claim evidence references, any canonical provider-supplied ID, and any non-initial discriminant. Sort normalized claims by content and deterministic provider/run/local-reference tie-breakers, then assign canonical IDs monotonically as `claim-0001`, `claim-0002`, and so on. Merge exact normalized claim duplicates while persisting every origin in `claim_origins`; providers never supply canonical IDs. Initial processing is the only v0.1 path that creates claims.
...
Send the same explicit compact board and canonical review set to fresh stateless Codex and Claude calls under `CrossExaminationPhaseResponse`. Require exactly one `ACCEPT`, `DISPUTE`, or `UNCERTAIN` stance per review-set canonical claim, including other-provider origins; reject missing, extra, or duplicate stances. Validate `existingEvidenceIds` only against the input board and `newEvidenceLocalIds` only against unique `newEvidence` declarations in that same response/run. Reject wrong namespaces, dangling/cross-run IDs, and every `claims` field. Mechanically validate/canonicalize new evidence, translate its stance links, and persist the next versioned board; disagreement creates rationale/evidence, not later claims. Run another round only for material claims still disputed or uncertain and only below `config.maxRounds`. Persist each round's phase and input/output board versions plus each agent run's frozen concrete model selection, phase, purpose, exact bounded discriminated request/response, board links, evidence, and stance before scheduling another call.
...
Collect independent `FinalPhaseResponse` positions in fresh stateless calls, requiring exactly one final stance for every canonical claim in the final input board. Apply the same existing-canonical versus response-local-new evidence validation and translation rules as cross-examination; reject every `claims` field and every missing, extra, duplicate, dangling, cross-run, or wrong-namespace reference. Ignore all earlier stances when classifying; they never substitute for a missing final stance. Apply the exhaustive pure function: `ACCEPT` + `ACCEPT` = `CONSENSUS`; `ACCEPT` + `DISPUTE` in either order = `DISAGREEMENT`; `DISPUTE` + `DISPUTE` = `REJECTED`; every pair containing `UNCERTAIN`, every missing/failed/cancelled agent, and anything other than exactly two valid final stances = `UNRESOLVED`. Mark a consensus with no `VERIFIED` evidence as `UNSUPPORTED`.
...
**Interfaces:**
- Consumes: `AskService`, `DebateService`, `AgentRegistry`, configured concrete model selections, project repositories, session and deliberation repositories, and `ActiveRuns`.
- Produces: Discord commands `/projects`, `/models`, `/switch`, `/ask`, `/debate`, `/status`, `/stop` and a testable interaction port.
...
Test unauthorized guild, unauthorized user, missing active project, `/models`, `/switch`, each `/ask` selection, `/debate topic:<text> project:<id?>` with active and explicit projects, separate Codex/Claude concrete model choices, omitted model choices with configured/default-provider behavior, unknown classes rejected before orchestration, `/status`, cancellation ownership, one-agent debate failure, deterministic debate rendering, unsupported consensus labeling, persisted model selection display, and a result longer than Discord's message limit.
...
- `/ask agent` required choice (`codex`, `claude`, `both`), `question` required string, `codex_model` optional configured Codex concrete-class choice, and `claude_model` optional configured Claude concrete-class choice
- `/debate topic` required string, `project` optional registered-project string, `codex_model` optional configured Codex concrete-class choice, and `claude_model` optional configured Claude concrete-class choice
- `/status`
- `/stop run` optional string defaulting to the requester's current run

Register the at-most-25 configured classes as provider-specific Discord choices, omitting a provider's model option entirely when its allowlist is empty. Omission is not displayed as a synthetic class: it uses `defaultModel` or provider default. Never expose raw CLI IDs as input values, and revalidate submitted choices to reject stale/forged interactions before orchestration. `authorize(scope, config)` must reject DMs, guilds outside `guildIds`, and users outside `allowedUserIds` before any database access.
...
Test exact command parsing for `setup`, `doctor`, and `start`, plus invalid commands. Unit-test executable resolution in this strict order: configured explicit native path, direct native executable in a `PATH` segment, then narrow documented native candidates. On Windows, reject configured or discovered `.cmd`/`.bat` files, treat `%APPDATA%\npm\claude.cmd` only as diagnostic evidence, and select `%APPDATA%\npm\node_modules\@anthropic-ai\claude-code\bin\claude.exe` or `%USERPROFILE%\.local\bin\claude.exe` only when it is a regular file whose direct bounded-runner `--version` spawn succeeds. Add the regression where a 160-byte npm `.cmd` shim exists but only its native package `.exe` target is selected and executed without `cmd.exe`. Cover the provider-documented `~/.local/bin/claude` native launcher on macOS and Linux; assert absent environment variables, missing shim targets, non-regular/non-executable files, and failed direct version probes produce actionable diagnostics. Tests must prove no shim parsing, recursive home scan, credential/session read, shell invocation, or `PATH` mutation is attempted. The vertical-slice test must create a temporary Git project and database, configure concrete Codex/Claude classes, use fake Codex and Claude CLI processes, invoke the Discord-neutral `/models`, `/ask both`, and `/debate` handlers, and assert:
...
Use `node:readline/promises` in `setup` to collect Discord application ID, guild IDs, authorized user IDs, one or more project ID/name/root triples, optional explicit provider native executable paths, and optional concrete model class/CLI ID/requested-effort/accepted-observation mappings plus `defaultModel` for each provider. Default to empty model lists and unset defaults. Show Codex `sol`/`terra`/`luna` and Claude `opus`/`fable`/`sonnet`/`haiku` mappings only as optional examples, never entitlement claims. Validate each project with Task 3 and all selections with Task 2 before saving. Resolve executables by configured explicit native path first, direct native `PATH` lookup second, and narrow documented native candidates third. Setup rejects Windows `.cmd`/`.bat` paths and may offer only a regular, directly version-probed native executable; it saves that path only after showing the portable form and receiving explicit confirmation and never mutates `PATH`. Create `.env` only after explicit confirmation, write only `AI_WORKSPACE_DISCORD_TOKEN=<entered value>`, set restrictive permissions where supported, and never echo the token.

- [ ] **Step 4: Implement doctor and startup composition**

`resolveAgentCommand()` never invokes a shell. It validates a configured explicit native path first, checks each `PATH` directory directly second, and checks only narrow native candidates third. Windows accepts `.exe` only: `%APPDATA%\npm\node_modules\@anthropic-ai\claude-code\bin\claude.exe` for a global npm installation and the provider-documented `%USERPROFILE%\.local\bin\claude.exe`. A `%APPDATA%\npm\claude.cmd` or any `.cmd`/`.bat` path is never selected, persisted, parsed, or executed; its presence is diagnostic-only and suggests the generic native npm target or an explicit native configuration path. macOS/Linux use native executable candidates such as `~/.local/bin/claude`. Require a regular file (and executable permission on Unix), then directly spawn `<candidate> --version` through `runProcess` with the probe bounds and an argument array before accepting it. Do not recurse through a home directory, read credentials or session files, invoke `cmd.exe`, or alter `PATH`. Return the resolution source and a safe actionable diagnostic when unresolved.

`doctor` prints OS, Node, Git, config path, database writability, each project validation result, verified native executable resolution source, and each agent's capability diagnostics. It validates configured model/effort flag support, safely knowable allowed effort values, observed-model reporting, and Claude inline fallback-control compatibility from version/help data without making a paid inference call. It explicitly reports that managed/server settings outrank `--settings`, fallback may run and incur cost before post-execution class validation, account entitlement and effective managed settings are runtime-only without a safe official contract, and provider-default executions are unverified. If only a Windows shim exists, its native target is missing, or a verified native candidate exists outside the current process `PATH`, doctor provides an actionable native-path/setup diagnostic and does not mutate the environment. It redacts home-relative details from shareable output unless `--verbose` is introduced in a separate design.
...
`README.md` must link `docs/decisions/README.md` and contain prerequisites for all three operating systems, clone/install commands, Discord application creation, `.env` setup, `pnpm setup`, `pnpm doctor`, `pnpm start`, `/models`, `/ask`, and `/debate topic:<text> project:<id?> codex_model:<class?> claude_model:<class?>` examples, local data locations, separate Codex and Claude CLI installation/authentication links, portable native-executable resolution and Windows shim troubleshooting, source-non-modification guarantees, the complete-host-read-isolation limitation, structured deliberation and deterministic-verdict guarantees, mechanical evidence limitations, model entitlement/observation limitations, and troubleshooting. Document configurable concrete model classes with Codex `sol`/`terra`/`luna` and Claude `opus`/`fable`/`sonnet`/`haiku` as non-entitlement examples, provider default by omission, fixed requested class per provider, same-class alias evolution, optional exact pinning, literal accepted-observation policies, ordinary local fallback-setting neutralization, managed-settings precedence and possible fallback cost before post-execution rejection, model-verification failures, and requested-not-observed effort with fail-closed allowed-value validation. `SECURITY.md` documents the allowlist, Git-tracked-only symlink rule, provider capability gate, ambient-config/fallback isolation flags and managed-policy limitation, post-execution model-class verification, integrity backstop, host read-isolation limitation, secret handling, and vulnerability reporting. `CONTRIBUTING.md` documents pnpm, TDD, quality commands, ADR usage, English-only public artifacts, and the no-secret rule.
...
Expected on the current machine: Git and Node pass; each CLI reports its verified native command source plus supported hardened safety/model/effort/observation capabilities or an actionable installation, path, authentication, or missing-flag diagnostic. A Windows npm `.cmd` shim is diagnostic only; its generic native package `.exe` target may be directly probed and offered for setup confirmation when valid, never silently persisted. Doctor makes no paid model call, labels entitlement/effective managed settings/runtime observation as unknown without safe contracts, warns that managed fallback may incur cost before post-execution rejection, and marks provider-default execution unverified. Do not weaken tests or adapter policy to make an unavailable or unsafe local CLI appear healthy.
...
- `pnpm doctor` reports verified native executable resolution plus real provider safety, model, effort, and observation capabilities and fails closed for unsupported source-non-modification contracts without paid inference. It reports that managed Claude settings outrank inline fallback controls and that fallback work/cost can precede post-execution rejection.
- Executable resolution checks configured explicit native paths, direct native `PATH` entries, and only narrow native platform candidates in that order; it rejects Windows `.cmd`/`.bat`, directly version-probes every accepted regular executable through the bounded runner, and never parses shims, recursively scans home, reads provider credentials/sessions, invokes a shell, or mutates `PATH`.
...
- Both CLI version/capability probes fail closed if any required safety, ambient-isolation, session, model, effort, observation, or structured-output contract is absent; explicit effort requires a reported allowed-values list containing it before spawn. Claude uses `--bare`, inline ordinary-settings fallback neutralization, the exact Read/Glob/Grep allowlist, explicit `mcp__*` denial, plan permission mode, no session persistence, print mode, and JSON output.
...
- Model IDs and requested efforts are direct inert argument-array elements; one immutable `ResolvedModelSelection` per provider reaches every debate phase. Tests prove Claude's inline settings neutralize ordinary user/project/local fallback configuration and that managed-like cross-class execution is detected only after execution from normalized JSON `modelUsage`; mismatches or absent observations fail with stable codes while retaining safe usage/cost diagnostics. Provider-default remains unverified, and AI Workspace never performs a silent application-level fallback.
