...
function addModelOption(
  ...
  command.addStringOption((option) =>
    option.setName(name).setDescription(`${label} configured model class`).setRequired(false).addChoices(...selections.map((selection) => ({ name: selection.class, value: selection.class }))),
  );
}
...
export function createSlashCommands(models: ProviderModels) {
  const ask = new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName("agent").setDescription("Agent selection").setRequired(true).addChoices(
      { name: "Codex", value: "codex" }, { name: "Claude", value: "claude" }, { name: "Both", value: "both" },
    ))
  ...
  const debate = new SlashCommandBuilder()
    ...
    .addStringOption((option) => option.setName("project").setDescription("Registered project ID").setRequired(false));
  ...
  return [
    new SlashCommandBuilder().setName("projects").setDescription("List registered projects"),
    new SlashCommandBuilder().setName("models").setDescription("List configured model classes"),
    new SlashCommandBuilder()
      .setName("switch")
      .setDescription("Switch the active project")
      .addStringOption((option) => option.setName("project").setDescription("Registered project ID").setRequired(true)),
    ask,
    debate,
    new SlashCommandBuilder().setName("status").setDescription("Show your active runs"),
    new SlashCommandBuilder()
      .setName("stop")
      .setDescription("Stop your current run")
      .addStringOption((option) => option.setName("run").setDescription("Run ID").setRequired(false)),
  ];
}
