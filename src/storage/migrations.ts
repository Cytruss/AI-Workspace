...
CREATE TABLE messages (id TEXT PRIMARY KEY, session_id TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE, role TEXT NOT NULL, agent_id TEXT, content TEXT NOT NULL, created_at TEXT NOT NULL);
...
CREATE TABLE debate_rounds (id TEXT PRIMARY KEY, session_id TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE, round_number INTEGER NOT NULL CHECK (round_number > 0), phase TEXT NOT NULL, status TEXT NOT NULL, input_board_id TEXT, output_board_id TEXT, created_at TEXT NOT NULL, finished_at TEXT, UNIQUE (session_id, round_number, phase), UNIQUE (id, session_id), FOREIGN KEY (input_board_id, session_id) REFERENCES claim_boards(id, session_id), FOREIGN KEY (output_board_id, session_id) REFERENCES claim_boards(id, session_id));
CREATE TABLE agent_runs (id TEXT PRIMARY KEY, session_id TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE, agent_id TEXT NOT NULL, requested_model_class TEXT, requested_model_id TEXT, requested_effort TEXT, observed_model_ids_json TEXT NOT NULL, model_verification TEXT NOT NULL, round_id TEXT, phase TEXT NOT NULL, purpose TEXT NOT NULL, input_board_id TEXT, output_board_id TEXT, request_json TEXT NOT NULL, response_json TEXT, status TEXT NOT NULL, exit_code INTEGER, duration_ms INTEGER NOT NULL, diagnostics_json TEXT NOT NULL, created_at TEXT NOT NULL, finished_at TEXT, UNIQUE (id, session_id), CHECK ((requested_model_class IS NULL AND requested_model_id IS NULL AND requested_effort IS NULL) OR (requested_model_class IS NOT NULL AND requested_model_id IS NOT NULL)), CHECK (model_verification IN ('verified', 'unverified')), FOREIGN KEY (round_id, session_id) REFERENCES debate_rounds(id, session_id), FOREIGN KEY (input_board_id, session_id) REFERENCES claim_boards(id, session_id), FOREIGN KEY (output_board_id, session_id) REFERENCES claim_boards(id, session_id));
